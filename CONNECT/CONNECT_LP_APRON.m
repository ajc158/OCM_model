...
state.port = 5589;
state.outputs.out1.dims = [32 32];


sys = sys.addprocess('APRON', 'dev/abrg/2009/apronClient', fS, state);

sys = sys.link('vis/LIP/out>out', 'APRON<in',0);
sys = sys.link('orient/SG>inhib', 'APRON<in2',0);

sys = sys.link('APRON>out1', 'orient/SG_in/act<in',0);
