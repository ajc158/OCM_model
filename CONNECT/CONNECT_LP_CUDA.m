...
projState.norm = 'aff';
sys = nw_modAddProj(sys, fS, 'orient.retina_in', 'orient.retina_1', 2.0, [p2p 'gaussianIIR'], projState, 'add');
sys = nw_modAddProj(sys, fS, 'orient.retina_in', 'orient.retina_2', 2.0, [p2p 'gaussianIIR'], projState, 'add');
...
sys = nw_modAddInput(sys, fS/10, 'visResampIn>ch1', 'vis.H_V1m', -1.0, [p2p 'gaussianIIR'], projState, 'add');
projState.type = 'second_derivative_vert';
sys = nw_modAddInput(sys, fS/10, 'visResampIn>ch1', 'vis.V_V1m', -1.0, [p2p 'gaussianIIR'], projState, 'add');



state = {};
state.IPaddress = '192.168.101.60';
state.port = 5589;


sys = sys.addprocess('APRON', 'dev/abrg/2010/oculo', fS, state);

sys = sys.link('vis/LIP/out>out', 'APRON<in',0);
% sys = sys.link('orient/SG>inhib', 'APRON<in2',0);

sys = sys.link('APRON>out', 'orient/SG_in/act<in',0);
