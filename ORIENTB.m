...
orient = nw_modAddPop(orient, 'retina_in', fS, [layerDims], {[layerLocs]}, [p2a 'leaky'], actpars, [p2o 'linear'], outpars);
orient = nw_modAddPop(orient, 'retina_1', fS, [layerDims], {[layerLocs]}, [p2a 'leaky'], actpars, [p2o 'abs'], outpars);
...
if strcmp(taskType,'TEST_ROBOT')
    state.xRange2 = 26/50;
    state.yRange2 = 26/50;
    state.xRange = degrees_to_edgeP*2 * 1.6;
    state.yRange = degrees_to_edgeP*2 * 1.6;% 1.1 is a fix factor
    state.mode = 1.0;
    state.quadraticFactor = 0.0006;%0.00146
...
if fovea_striatum_comp_v2 == true
    ...
    for i = 1:layerDims(1)
        for j = 1:layerDims(2)
            ...
            projState.weights(num) = single(tanh(j./2-layerDims(1)/6+1).*0.5+0.5);
        end
    end
...
% orient to orient ------------------------------------------------------------
projState = defParsProj;
orient = nw_modAddProj(orient, fS, 'SC_sup', 'THAL', SC_SUPERIOR_TO_THAL, [p2p 'onetoone'], projState, 'add_no_shunt');
