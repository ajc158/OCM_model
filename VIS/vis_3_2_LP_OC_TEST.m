...
off_fact = 2*ret_dims(1)/In_dims(1);
...
elements = {'H_V2' 'TL_V2' 'TR_V2' 'BL_V2' 'BR_V2' 'V_V2'};
...
vis = nw_modAddProj(vis, fS, 'V_V1m', 'V_V2',  normF*W_F/1.5, [p2p 'gaussianLPF'], projState, 'add');
...
%         if i ~= j
            eval(['vis = nw_modAddProj(vis, fS, ''' cell2mat(elements(i)) ''',''' cell2mat(elements(j)) ''', W_I, [p2p ''onetoone''], projState, ''shunt_div'');']);
%         end
    end
end
...
vis = nw_modAddProj(vis, fS, 'TL_V2m', 'o_V4', normF*W_F/oFactor, [p2p 'gaussianLP'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'TL_V2m', 'c_V4', normF*W_F/4, [p2p 'gaussianLP'], projState, 'add');
projState.offset = [1.0 1.0].*off_fact;
vis = nw_modAddProj(vis, fS, 'H_V2m', 'Rc_V4', normF* W_F/4, [p2p 'gaussianLP'], projState, 'add');

projState.offset = [-1 1.0].*off_fact;
vis = nw_modAddProj(vis, fS, 'BL_V2m', 'o_V4', normF* W_F/oFactor, [p2p 'gaussianLP'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'BL_V2m', 'c_V4', normF* W_F/4, [p2p 'gaussianLP'], projState, 'add');
projState.offset = [-1.0 1.0].*off_fact;
vis = nw_modAddProj(vis, fS, 'H_V2m', 'Rc_V4',  normF*W_F/4, [p2p 'gaussianLP'], projState, 'add');

projState.offset = [1.0 -1].*off_fact;
vis = nw_modAddProj(vis, fS, 'TR_V2m', 'o_V4',  normF*W_F/oFactor, [p2p 'gaussianLP'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'TR_V2m', 'Rc_V4', normF* W_F/4, [p2p 'gaussianLP'], projState, 'add');
projState.offset = [1.0 -1.0].*off_fact;
vis = nw_modAddProj(vis, fS, 'H_V2m', 'c_V4',  normF*W_F/4, [p2p 'gaussianLP'], projState, 'add');

projState.offset = [-1.0 -1].*off_fact;
vis = nw_modAddProj(vis, fS, 'BR_V2m', 'o_V4',  normF*W_F/oFactor, [p2p 'gaussianLP'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'BR_V2m', 'Rc_V4',  normF*W_F/4, [p2p 'gaussianLP'], projState, 'add');
projState.offset = [-1.0 -1.0].*off_fact;
vis = nw_modAddProj(vis, fS, 'H_V2m', 'c_V4',  normF*W_F/4, [p2p 'gaussianLP'], projState, 'add');
...
for i = 1:size(elements,2)
    eval(['vis = nw_modAddProj(vis, fS, ''' cell2mat(elements(i)) ''',''' cell2mat(elements(i)) ''', W_R, [p2p ''onetoone''], projState, ''add'');']);
    eval(['vis = nw_modAddProj(vis, fS, ''' cell2mat(elements(i)) 'b'',''' cell2mat(elements(i)) ''', W_A*0, [p2p ''onetoone''], projState, ''add'');']);
    for j = 1:size(elements,2)
%         if i ~= j
            projState.sigma = 4.0;
            projState.norm = 'none';
...
projState.norm = 'none';
projState.position = 2;
vis = nw_modAddProj(vis, fS, 'ITfb', 'n2_ITb', W_B, [p2p 'mux'], projState, 'add');
projState.position = 3;
vis = nw_modAddProj(vis, fS, 'ITfb', 'n3_ITb', W_B, [p2p 'mux'], projState, 'add');
projState.position = 4;
vis = nw_modAddProj(vis, fS, 'ITfb', 'n5_ITb', W_B, [p2p 'mux'], projState, 'add');
projState.position = 5;
vis = nw_modAddProj(vis, fS, 'ITfb', 'n6_ITb', W_B, [p2p 'mux'], projState, 'add');
projState.position = 6;
vis = nw_modAddProj(vis, fS, 'ITfb', 'n8_ITb', W_B, [p2p 'mux'], projState, 'add');
projState.position = 7;
vis = nw_modAddProj(vis, fS, 'ITfb', 'n9_ITb', W_B, [p2p 'mux'], projState, 'add');
projState.position = 8;
vis = nw_modAddProj(vis, fS, 'ITfb', 'o_V4b', W_B, [p2p 'mux'], projState, 'add');
projState.position = 9;
vis = nw_modAddProj(vis, fS, 'ITfb', 'c_V4b', W_B, [p2p 'mux'], projState, 'add');
projState.position = 10;
vis = nw_modAddProj(vis, fS, 'ITfb', 'Rc_V4b', W_B, [p2p 'mux'], projState, 'add');

projState = [];
projState.norm = 'none';

vis = nw_modAddProj(vis, fS, 'n2_IT', 'n2_ITb', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'n3_IT', 'n3_ITb', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'n5_IT', 'n5_ITb', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'n6_IT', 'n6_ITb', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'n8_IT', 'n8_ITb', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'n9_IT', 'n9_ITb', 1, [p2p 'onetoone'], projState, 'shunt');

%% %%%%% V4
vis = nw_modAddProj(vis, fS, 'Rc_V4', 'Rc_V4b', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'c_V4', 'c_V4b', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'o_V4', 'o_V4b', 1, [p2p 'onetoone'], projState, 'shunt');
...
vis = nw_modAddProj(vis, fS, 'n8_ITb', 'o_V4b', W_B/2, [p2p 'discMax'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'n3_ITb', 'Rc_V4b', W_B/2, [p2p 'discMax'], projState, 'add');

vis = nw_modAddProjMult(vis, fS, 'n9_ITb', {'o_V4b' 'Rc_V4b'}, W_B/2, [p2p 'discMax'], projState, 'add');

vis = nw_modAddProjMult(vis, fS, 'n6_ITb', {'o_V4b' 'c_V4b'}, W_B/2, [p2p 'discMax'], projState, 'add');

vis = nw_modAddProjMult(vis, fS, 'n5_ITb', {'c_V4b' 'Rc_V4b'}, W_B/2, [p2p 'discMax'], projState, 'add');

vis = nw_modAddProjMult(vis, fS, 'n2_ITb', {'c_V4b' 'Rc_V4b'}, W_B/2, [p2p 'discMax'], projState, 'add');

%% %%%%% V2

vis = nw_modAddProj(vis, fS, 'TR_V2', 'TR_V2b', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'TL_V2', 'TL_V2b', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'BR_V2', 'BR_V2b', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'BL_V2', 'BL_V2b', 1, [p2p 'onetoone'], projState, 'shunt');
vis = nw_modAddProj(vis, fS, 'H_V2', 'H_V2b', 1, [p2p 'onetoone'], projState, 'shunt');

projState.radius = 8;

vis = nw_modAddProjMult(vis, fS, 'o_V4b', {'TR_V2b' 'BR_V2b' 'TL_V2b' 'BL_V2b'}, W_B/4, [p2p 'discMax'], projState, 'add');

vis = nw_modAddProjMult(vis, fS, 'c_V4b', {'TL_V2b' 'BL_V2b'}, W_B/3, [p2p 'discMax'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'c_V4b', 'H_V2b', W_B/8, [p2p 'discMax'], projState, 'add');

vis = nw_modAddProjMult(vis, fS, 'Rc_V4b', {'TR_V2b' 'BR_V2b'}, W_B/3, [p2p 'discMax'], projState, 'add');
vis = nw_modAddProj(vis, fS, 'Rc_V4b', 'H_V2b', W_B/8, [p2p 'discMax'], projState, 'add');
