function sys = nw_modAddOutput(sys, fS, src, dst, weight, projClass, projState, setName)
...
if ~isfield(projState, 'dstDims')
    error('state structure must contain the field dstDims');
end
if ~isfield(projState, 'dstLocs')
    defaultState.dstLocs = {[1,1]};
end
...
for i = 1:subCount
    ...
    if i == 1
        %add proj process to dst pathsystem and link to activation process
           ...
           child_sys = child_sys.link(['out>out'], [proj_name '<in']);
        child_sys = child_sys.expose([proj_name '>out'], [proj_name '_out']);
    else
        child_sys = child_sys.expose([src_names{i-1} '>' proj_name '_out'], [proj_name '_out']);
    end
...
for i = 2:subCount
    if i == 2
        child_sys = dst_children{2};
        child_sys = child_sys.expose([dst_names{1} dstPort], safe_dst_port);
    else
        child_sys = child_sys.expose([dst_names{i-1} '>' safe_dst_port], safe_dst_port);
    end
...
%link dst to newly exposed output
if subCount < 2
    sys = sys.link([src_names{end} '>' proj_name '_out'], [dst_names{end} dstPort]);
else
    sys = sys.link([src_names{end} '>' proj_name '_out'], [dst_names{end} '<' safe_dst_port]);
end
