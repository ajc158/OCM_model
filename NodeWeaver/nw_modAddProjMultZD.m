function sys = nw_modAddProjMult(sys, fS, src, dst, weight, projClass, projState, setName)
...
proj_name = generateUniqueName(sys, src, dst{1});

%strip off enclosing subsystems
for i = 1:size(dst,2)
    [dst_namesT, dst_pathsT, dst_parentsT, dst_childrenT] = parsepaths(dst{i}, sys);
    dst_names{i} = dst_namesT;
    dst_paths{i} = dst_pathsT;
    dst_parents{i} = dst_parentsT;
    dst_childen{i} = dst_childrenT;
...
defaultState.dstDims = eval(['sys.' dst{1} '.act.state.dims']);
defaultState.dstLocs = eval(['sys.' dst{1} '.act.state.locs']);
...
subCount = length(dst_paths{1});
child_sys = eval(['sys.' dst{1}]);
for i = 1:subCount
    parent_sys = dst_parents{1}{i};
    parent_sys = parent_sys.removesubsystem(dst_names{1}{i});
    if i == 1
        ...
        child_sys = child_sys.expose([dst_names{1}{i-1} '<' proj_name '_in'], [proj_name '_in']);
    end
    child_sys = parent_sys.addsubsystem(dst_names{1}{i}, child_sys);
end
sys = child_sys;
...
if subCount < 2
    sys = sys.link([src_names{end} '>' 'out'], [dst_names{1}{end} '<' proj_name '_in'],0);
else
    sys = sys.link([src_names{end} '>' sanatize(src) '_out'], [dst_names{1}{end} '<' proj_name '_in'],0);
end
   

for j = 2:size(dst,2)
   
    % link up the other destinations
    sys = sys.link([dst{1} '/' proj_name '>out'], [dst{j} '/act<<' setName],0);
