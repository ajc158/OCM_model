...
if subCount < 2
    sys = sys.link([src_names{end} '>' 'out'], [dst_names{1}{end} '<' proj_name '_in']);
else
    sys = sys.link([src_names{end} '>' sanatize(src) '_out'], [dst_names{1}{end} '<' proj_name '_in']);
end
...
for j = 2:size(dst,2)
    ...
    sys = sys.link([dst{1} '/' proj_name '>out'], [dst{j} '/act<<' setName]);
