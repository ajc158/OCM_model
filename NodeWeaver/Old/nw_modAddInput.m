...
for i = 1:subCount
    ...
    if i == 1
        %add proj process to dst pathsystem and link to activation process
           ...
           child_sys = child_sys.link([proj_name], ['act<<' setName]);
...
for i = 2:subCount
    if i == 2
        ...
        child_sys = child_sys.expose([src_names{1}], [sanatize(src) '_out']);
    else
...
if subCount < 2
    sys = sys.link([src_names{end} '>' 'out'], [dst_names{end} '<' proj_name '_in']);
else
    sys = sys.link([src_names{end} '>' sanatize(src) '_out'], [dst_names{end} '<' proj_name '_in']);
end

function proj_name = generateUniqueName(sys, src, dst)

% this totally fails! Now adding random number from 0 to 100!
proj_name = [src '__2__' dst];
proj_name = sanatize(proj_name);
name_taken = 1;
name_index = round(rand*100);
%while name_taken
%    try
%        eval(['sys.' proj_name '_' num2str(name_index)]);
%    catch
