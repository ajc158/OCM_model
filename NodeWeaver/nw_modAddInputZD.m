function sys = nw_modAddInput(sys, fS, src, dst, weight, projClass, projState, setName)
...
if subCount < 2
    sys = sys.link([src_names{end} srcPort], [dst_names{end} '<' proj_name '_in'],0);
else
    sys = sys.link([src_names{end} '>' safe_src_port], [dst_names{end} '<' proj_name '_in'],0);
end
