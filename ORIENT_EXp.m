...
if fovea_striatum_comp_v2 == true
    ...
    for i = 1:layerDims(1)
        for j = 1:layerDims(2)
            ...
            projState.weights(num) = single(tanh(j./2-layerDims(1)/4+ fovea_damping_offset).*0.5+0.5);
        end
    end
...
% orient to orient ------------------------------------------------------------
projState = defParsProj;
orient = nw_modAddProj(orient, fS, 'SC_sup', 'FEF', SC_SUPERIOR_TO_THAL, [p2p 'onetoone'], projState, 'add');
...
orient = nw_modAddProj(orient, fS, 'BG.SNr', 'THAL', SNR_TO_THAL_D, [p2p 'onetoone'], projState, 'shunt_div');
